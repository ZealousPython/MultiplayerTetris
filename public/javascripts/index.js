...
let garbageGen = new Math.seedrandom(seed + "garbage1s")
let score = 0
...
function resetBoard() {
  ...
  queue = [];
  positionInQueue = 0;
  generator = new Math.seedrandom(seed);
  garbageGen = new Math.seedrandom(seed + "garbages")
  currentPiece = null
  nextSeven();
  generateAlot();
  score = 0
...
function soloPlay(){
  conn = null
  peer.destroy()
    peer = new Peer(options = { host: ip, port: '9000', path: '/myapp', debug: 2 })
  resetBoard()
}
function nextSeven() {
